...
type ValidationManager<'T when 'T :> ComputedData>
    ...
    let validateInteractiveDetachedCore storeResult (entities: struct (Entity * Lazy<'T>) list) =
    ...
    /// after an editor resource has been committed and stores a bounded reusable result.
    let validateInteractiveDetached entities =
        validateInteractiveDetachedCore true entities

    /// Validate prepared overlay entities without changing ErrorCache or the
    /// detached-result cache. Live workspace services are read-only context.
    let validateInteractiveDetachedPure entities =
        validateInteractiveDetachedCore false entities
    ...
    member _.ValidateInteractiveDetachedPure(entities: struct (Entity * Lazy<'T>) list) =
        validateInteractiveDetachedPure entities
