...
module LocalisationString =
    ...
    let private ref =
        between (skipChar '$') (skipChar '$') (dollarChars .>> optional dollarColour |>> Ref)

    let private command =
        between (skipChar '[') (skipChar ']') (commandChars .>> optional commandFormat |>> Command)

    let private locStringParser =
        many (valueChars |>> Chars <|> ref <|> command) .>> eof

    let private jominiCommand, jominiCommandImpl = createParserForwardedToRef ()

    let private simpleParam =
        chSkip '\'' >>. many1Satisfy (isNoneOf [ '.'; '\'' ]) .>> chSkip '\'' .>> ws
        |>> JominiLocCommandParam.Param // <!> "Param"

    let private simpleCommands =
        ((sepBy1 jominiCommand (chSkip '.')) |>> JominiLocCommandParam.Commands) // <!> "Commands"

    let private segment =
        many1Satisfy (isNoneOf [ '.'; '('; '|'; ']'; ')'; ' '; ',' ]) .>> ws
        |>> (fun k -> JominiLocCommand.Command(k, [])) //<!> "segment" //<!> "segment"

    let private pParams = sepBy1 (simpleParam <|> simpleCommands) (chSkip ',') // <!> "pParams"//<!> "pParams"

    let private pFunction =
        (many1Satisfy (isNoneOf [ '.'; ']'; '('; ')' ])) .>> (chSkip '(') .>>. pParams
        .>> chSkip ')'
        |>> (fun (key, cs) -> JominiLocCommand.Command(key, cs)) // <!> "function"
    ...
    let private jominiCommandWrapper =
        between
            (skipChar '[' .>> ws)
            (skipChar ']' .>> ws)
            ((sepBy1 jominiCommand (ch '.')) .>> optional commandFormat |>> JominiCommand)

    let private jominiLocStringParser =
        many (
            (valueChars |>> Chars <|> ref <|> jominiCommandWrapper)
            .>> optional (ch '#' .>> restOfLine false .>> ws)
        )
        .>> eof

    let private parseLocString fileString filename =
        runParserOnString locStringParser () filename fileString

    let private parseJominiLocString fileString filename =
    ...
    let validateProcessedLocalisationBase
        ...
        let validateContextResult (e: LocEntry) cr =
            ...
            | LocNotFound s ->
                Invalid(Guid.NewGuid(), [ invManual (ErrorCodes.InvalidLocCommand e.key s) e.position e.key None ])
            | LocNotFoundInType(s, dataType, confident) ->
                Invalid(
                    Guid.NewGuid(),
                    [ invManual (ErrorCodes.LocCommandNotInDataType e.key s dataType confident) e.position e.key None ]
                )
            | LocContextResult.NewScope s ->
                Invalid(
                    Guid.NewGuid(),
                    [ invManual
                          (ErrorCodes.CustomError
                              (sprintf "Localisation command does not end in a command but ends in scope %O" s)
                              Severity.Error)
                          e.position
                          e.key
                          None ]
            ...
            | LocContextResult.WrongScope(c, actual, expected: Scope list) ->
                Invalid(
                    Guid.NewGuid(),
                    [ invManual
                          (ErrorCodes.LocCommandWrongScope
                              c
                              (expected |> List.map (fun f -> f.ToString()) |> String.concat ", ")
                              (actual.ToString()))
                          e.position
                          e.key
                          None ]
            ...
            |> List.fold
                (fun s (k, v) ->
                    if v.desc == "\"REPLACE_ME\"" || v.desc == "\"TODO_CD\"" then
                        ...
                        <&&> Invalid(
                            Guid.NewGuid(),
                            [ invManual (ErrorCodes.ReplaceMeLoc v.key lang) v.position v.key None ]
                        )
                    else
                        s)
                OK
    ...
    let processLocalisationBase
        ...
        api
        |> (fun (lang, entries) -> lang, entries |> Map.map (fun _ entry -> processEntry entry))
    ...
    let validateLocalisationCommandsBase
        localisationCommandValidator
        (locentry: LocEntry)
    ...
    let validateJominiLocalisationCommandsBase
        ...
        let validateCommand (c: JominiLocCommand list) =
            ...
            | LocNotFound s ->
                Invalid(
                    Guid.NewGuid(),
                    [ invManual (ErrorCodes.InvalidLocCommand locentry.key s) locentry.position locentry.key None ]
                )
            | LocNotFoundInType(s, dataType, confident) ->
                Invalid(
                    Guid.NewGuid(),
                    [ invManual
                          (ErrorCodes.LocCommandNotInDataType locentry.key s dataType confident)
                          locentry.position
                          locentry.key
                          None ]
                )
    ...
    let private getRange (start: FParsec.Position) (endp: FParsec.Position) =
    ...
    let validateLocalisationSyntax (results: Results) =
        ...
        results.Values |> List.ofSeq |> List.filter (fun (v, _, _, _) -> not v)
        <&!&> (fun (_, _, error, pos) ->
            if pos.IsSome then
                Invalid(Guid.NewGuid(), [ createInvalid error pos ])
            else
                OK)
