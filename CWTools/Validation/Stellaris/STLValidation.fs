...
module STLValidation =
        ...
        fun _ es ->
            ...
            let eventToErrors (event: Node) =
                ...
                | Some trigger ->
                    trigger.Leaves
                    <&!&> (fun l ->
                        if Array.contains l.Key stellarisEventPreTriggers then
                            Invalid(Guid.NewGuid(), [ inv (ErrorCodes.PossiblePretrigger l.Key) l ])
                        else
                            OK)
            ...
            let popjobToError (popjob: Node) =
                ...
                | Some possible ->
                    possible.Leaves
                    <&!&> (fun l ->
                        if Array.contains l.Key stellarisPopJobPreTriggers then
                            Invalid(Guid.NewGuid(), [ inv (ErrorCodes.PossiblePretrigger l.Key) l ])
                        else
                            OK)
            ...
            (fun (x: Node) children ->
                let results =
                    ...
                    | "research_leader" ->
                        ...
                        | area2 when area <> area2 ->
                            Invalid(Guid.NewGuid(), [ inv (ErrorCodes.ResearchLeaderTech area area2) x ])
                        | _ -> OK
    ...
    let addGeneratedModifiers (modifiers: ActualModifier list) (es: STLEntitySet) : ActualModifier list =
        ...
        let shipModifierCreate =
            (fun k ->
                [ { ActualModifier.tag = "shipsize_" + k + "_build_speed_mult"
                    category = modifierCategoryManager.ParseModifier () "Starbases" }
                  // {tag = "shipsize_"+k+"_build_cost_mult"; category = modifierCategoryManager.ParseModifier() "Starbase" }
                  { tag = "shipsize_" + k + "_hull_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = "shipsize_" + k + "_hull_add"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = "shipsize_" + k + "_evasion_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = "shipsize_" + k + "_disengage_chance_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = "shipsize_" + k + "_tracking_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
        ...
        let weaponTags =
            es.GlobMatch("**/common/component_tags/*.txt")
            |> List.collect (fun f -> f.LeafValues |> List.ofSeq)

        let weaponTagsModifierCreate =
            (fun k ->
                [ { ActualModifier.tag = k + "_weapon_damage_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = k + "_weapon_fire_rate_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" }
                  { tag = k + "_speed_mult"
                    category = modifierCategoryManager.ParseModifier () "Ships" } ])
        ...
        let weaponModifiers =
            weaponTags
            |> List.map (fun l -> l.Value.ToRawString())
            ...
            seq {
                ...
                | Some mt ->
                    if mt.LeafValues |> Seq.exists (fun lv -> lv.Key == "mult") then
                        yield
                            { ActualModifier.tag = triggeredCat + res + modType + "_mult"
                    ...
                    if mt.LeafValues |> Seq.exists (fun lv -> lv.Key == "add") then
                        yield
                            { ActualModifier.tag = triggeredCat + res + modType + "_add"
                              category = modifierCategoryManager.ParseModifier () "Economic Units" }
                    else
                        ()
                | None -> ()
        ...
        let econCategoryModifierCreateGenerator (node: Node) =
            ...
            (fun res ->
                [ match node.Child "generate_mult_modifiers" with
                  ...
                  | None -> ()
                  yield!
                      (node.Childs "triggered_upkeep_modifier"
                       |> Seq.collect (econCategoryTriggeredModifierCreate "_upkeep" res))
                  yield!
                      (node.Childs "triggered_cost_modifier"
                       |> Seq.collect (econCategoryTriggeredModifierCreate "_cost" res))
                  yield!
                      (node.Childs "triggered_produces_modifier"
                       |> Seq.collect (econCategoryTriggeredModifierCreate "_produces" res)) ])

        let economicCategories = es.GlobMatchChildren("**/common/economic_categories/*.txt")

        let baseEconCategoryModifiersCreates =
            economicCategories |> List.map econCategoryModifierCreateGenerator
        ...
        let srModifiers =
            (baseEconCategoryModifiersCreates |> List.collect (fun f -> f ""))
            @ (srKeys |> List.collect srModifierCreate)

        let pop_cats =
            es.GlobMatchChildren("**/common/pop_categories/*.txt")
            |> List.map (fun f -> f.Key)

        let popCatModifierCreate =
            (fun k ->
                [ { ActualModifier.tag = "pop_cat_" + k + "_happiness"
                    category = modifierCategoryManager.ParseModifier () "Pops" }
                  { tag = "pop_cat_" + k + "_political_power"
                    category = modifierCategoryManager.ParseModifier () "Pops" } ])

        let popCatModifiers = pop_cats |> List.collect popCatModifierCreate

        let jobs =
            es.GlobMatchChildren("**/common/pop_jobs/*.txt") |> List.map (fun f -> f.Key)
        ...
        let pcModifiers =
            pcKeys
            |> List.map (fun k ->
                { ActualModifier.tag = k + "_habitability"
                  category = modifierCategoryManager.ParseModifier () "Habitability" })

        let operationTags =
            es.GlobMatchChildren("**/common/espionage_operation_types/*.txt")
            |> List.map (fun o -> o.Key)

        let operationCatTags =
            es.GlobMatchChildren("**/common/espionage_operation_categories/*.txt")
            |> List.map (fun o -> o.Key)

        let operationTagModifierCreate =
            (fun k ->
                [ { ActualModifier.tag = k + "_speed_mult"
                    category = modifierCategoryManager.ParseModifier () "Espionage" }
                  { ActualModifier.tag = k + "_difficulty_add"
                    category = modifierCategoryManager.ParseModifier () "Espionage" }
                  { ActualModifier.tag = "hostile_" + k + "_difficulty_add"
                    category = modifierCategoryManager.ParseModifier () "Espionage" }
                  { ActualModifier.tag = "hostile_" + k + "_difficulty_mult"
                    category = modifierCategoryManager.ParseModifier () "Espionage" }
                  { ActualModifier.tag = k + "_skill_add"
                    category = modifierCategoryManager.ParseModifier () "Espionage" }
                  { ActualModifier.tag = k + "_skill_mult"
                    category = modifierCategoryManager.ParseModifier () "Espionage" } ])

        let operationModifiers =
            (operationTags @ operationCatTags) |> List.collect operationTagModifierCreate

        let buildingTags =
            es.GlobMatch("**/common/building_tags/*.txt")
            |> List.collect (fun f -> f.LeafValues |> List.ofSeq)

        let buildingTagModifierCreate =
            (fun k ->
                [ { ActualModifier.tag = k + "_construction_speed_mult"
                    category = modifierCategoryManager.ParseModifier () "Planets" }
                  { tag = k + "_build_cost_mult"
                    category = modifierCategoryManager.ParseModifier () "Planets" } ])

        let buildingModifiers =
            buildingTags
            |> List.map (fun l -> l.Value.ToRawString())
            |> List.collect buildingTagModifierCreate

        let buildingWithModCap =
            es.GlobMatchChildren("**/common/buildings/*.txt")
            |> List.filter (fun n -> n.TagText "is_capped_by_modifier" == "yes")

        let buildingWithModCapCreate =
            (fun k ->
                [ { ActualModifier.tag = k + "_max"
                    category = modifierCategoryManager.ParseModifier () "Planets" } ])

        let buildingWithModCapModifiers =
            buildingWithModCap
            |> List.map (fun n -> n.Key)
            |> List.collect buildingWithModCapCreate

        let countryTypeKeys =
            es.GlobMatchChildren("**/common/country_types/*.txt")
            |> List.map (fun f -> f.Key)

        let countryTypeModifiers =
            countryTypeKeys
            |> List.map (fun k ->
                { ActualModifier.tag = "damage_vs_country_type_" + k + "_mult"
                  category = modifierCategoryManager.ParseModifier () "Ships" })

        let speciesKeys =
            es.GlobMatchChildren("**/common/species_archetypes/*.txt")
        ...
        let speciesModifiersCreate =
            (fun (robotic: bool, k) ->
                [ yield
                      { ActualModifier.tag = k + "_species_trait_points_add"
                        category = modifierCategoryManager.ParseModifier () "Countries" }
                  yield
                      { ActualModifier.tag = k + "_pop_happiness"
                        category = modifierCategoryManager.ParseModifier () "Pops" }
                  if not robotic then
                      yield!
                          [ { ActualModifier.tag = k + "_pop_growth_speed"
                              category = modifierCategoryManager.ParseModifier () "Planets" }
                            { ActualModifier.tag = k + "_pop_growth_from_immigration"
                              category = modifierCategoryManager.ParseModifier () "Planets" }
                            { ActualModifier.tag = k + "_pop_growth_reduction"
                              category = modifierCategoryManager.ParseModifier () "Planets" } ]

                  ])

        let speciesModifiers = speciesKeys |> List.collect speciesModifiersCreate

        let districts =
            es.GlobMatchChildren("**/common/districts/*.txt")
            |> List.filter (fun d -> not (d.TagText "is_capped_by_modifier" == "no"))

        let districtModifiers =
            districts
            |> List.map (fun k ->
                { ActualModifier.tag = k.Key + "_max"
                  category = modifierCategoryManager.ParseModifier () "Planets" })

        let popEthicKeys =
            es.GlobMatchChildren("**/common/ethics/*.txt") |> List.map (fun s -> s.Key)

        let popEthicModifiers =
            popEthicKeys
            |> List.map (fun k ->
                { ActualModifier.tag = "pop_" + k + "_attraction_mult"
                  category = modifierCategoryManager.ParseModifier () "Pops" })

        let techCategoryKeys =
            es.GlobMatchChildren("**/common/technology/category/*.txt")
            |> List.map (fun s -> s.Key)

        let techCatModifiers =
            techCategoryKeys
            |> List.map (fun k ->
                { ActualModifier.tag = "category_" + k + "_research_speed_mult"
                  category = modifierCategoryManager.ParseModifier () "Countries" })
        ...
        @ pcModifiers
        @ buildingModifiers
        @ countryTypeModifiers
        @ speciesModifiers
        @ modifiers
        @ buildingWithModCapModifiers
        @ districtModifiers
        @ popEthicModifiers
        @ techCatModifiers
        @ operationModifiers
    ...
    let getTechnologies (es: STLEntitySet) =
        ...
        let inner =
            fun (t: Node) ->
                ...
                let prereqs =
                    t.Child "prerequisites"
                    |> Option.map (fun c -> c.LeafValues |> Seq.toList |> List.map (fun v -> v.Value.ToRawString()))
                    |> Option.defaultValue []
    ...
    let validateTechnologies: STLFileValidator =
        fun resourceAPI es ->
            ...
            let getPrereqs (b: Node) : obj list =
                ...
                | None -> []
                | Some p -> p.LeafValues |> List.ofSeq |> List.map (fun lv -> upcast lv.Value.ToRawString())
                ...
                if
                    List.contains
                        ...
                        [ EntityType.Buildings
                          EntityType.ShipSizes
                          EntityType.SectionTemplates
                          EntityType.ComponentTemplates
                          EntityType.StrategicResources
                          EntityType.Armies
                          EntityType.Edicts
                          EntityType.TileBlockers
                          EntityType.Deposits
                          EntityType.Decisions
                          EntityType.Traits ]
                then
                    List.collect getPrereqs e.entity.Children
                else
                    []
            ...
            let allPrereqs =
                os.AddOrGetCached "techprereqs" getPrereqGen
            ...
            let techChildren =
                techList
                |> List.map (fun (name, _) -> name, Set.contains name techPrereqs)
            ...
            let inner (t: Node) =
                ...
                let isWeightFactorZero =
                    t.Child "weight_modifier"
                    |> Option.map (fun wm ->
                        wm.Tag "factor"
                        |> (function
                        | Some(Value.Float 0.00m) -> true
                        | _ -> false))
                ...
                then
                    ...
                    Invalid(Guid.NewGuid(), [ inv (ErrorCodes.UnusedTech t.Key) t ])
    ...
    let validateShipDesigns: STLStructureValidator =
        fun os es ->
            ...
            let section_templates =
                os.AllOfTypeChildren EntityType.SectionTemplates
                @ es.AllOfTypeChildren EntityType.SectionTemplates

            let weapons =
                os.AllOfTypeChildren EntityType.ComponentTemplates
                @ es.AllOfTypeChildren EntityType.ComponentTemplates

            let getWeaponInfo (w: Node) =
                match w.Key with
                | "weapon_component_template" -> Some(w.TagText "key", ("weapon", w.TagText "size"))
                | "strike_craft_component_template" -> Some(w.TagText "key", ("strike_craft", w.TagText "size"))
                | "utility_component_template" -> Some(w.TagText "key", ("utility", w.TagText "size"))
                | _ -> None
            ...
            let getSectionInfo (s: Node) =
                ...
                | "ship_section_template" ->
                    ...
                    let smalls =
                        ...
                        |> (function
                        | Some(Value.Int i) -> createUtilSlot "SMALL_UTILITY_" "small" i
                        | _ -> [])

                    let med =
                        s.Tag "medium_utility_slots"
                        |> (function
                        | Some(Value.Int i) -> createUtilSlot "MEDIUM_UTILITY_" "medium" i
                        | _ -> [])

                    let large =
                        s.Tag "large_utility_slots"
                        |> (function
                        | Some(Value.Int i) -> createUtilSlot "LARGE_UTILITY_" "large" i
                        | _ -> [])

                    let aux =
                        s.Tag "aux_utility_slots"
                        |> (function
                        | Some(Value.Int i) -> createUtilSlot "AUX_UTILITY_" "aux" i
                        | _ -> [])
            ...
            let defaultTemplates =
                [ "DEFAULT_COLONIZATION_SECTION"; "DEFAULT_CONSTRUCTION_SECTION" ]

            let validateSection (s: Node) =
                let section = s.TagText "template"

                if defaultTemplates |> List.contains section then
                    OK
                else
                    ...
                    | Some smap -> s.Childs "component" <&!&> validateComponent section smap
    ...
    let validatePlanetKillers: STLStructureValidator =
        fun os es ->
            let planetkillers =
                es.AllOfTypeChildren EntityType.ComponentTemplates
                |> List.filter (fun ct -> ct.TagText "type" = "planet_killer")

            let onactions =
                (os.AllOfTypeChildren EntityType.OnActions
                 @ es.AllOfTypeChildren EntityType.OnActions)
                |> List.map (fun c -> c.Key)

            let scriptedtriggers =
                (os.AllOfTypeChildren EntityType.ScriptedTriggers
                 @ es.AllOfTypeChildren EntityType.ScriptedTriggers)
                |> List.map (fun c -> c.Key)
            ...
            let inner (node: Node) =
                ...
                (if List.exists ((==) on_action) onactions then
                     OK
                 else
                     Invalid(
                         Guid.NewGuid(),
                         [ inv
                               (ErrorCodes.PlanetKillerMissing(
                                   sprintf "Planet killer %s is missing on_action %s" key on_action
                               ))
                               node ]
                     ))
                <&&> (if List.exists ((==) trigger) scriptedtriggers then
                          OK
                      else
                          Invalid(
                              Guid.NewGuid(),
                              [ inv
                                    (ErrorCodes.PlanetKillerMissing(
                                        sprintf "Planet killer %s is missing scripted trigger %s" key trigger
                                    ))
                                    node ]
                          ))
                ...
                (fun (x: Node) children ->
                    if x.Key == "if" && x.Has "else" && not (x.Has "if") then
                        ...
                        let _, res =
                            nodes
                            |> List.fold
                                (fun (s, r: ValidationResult option) n -> if r.IsSome then s, r else checkNext s n)
                                (false, None)
    ...
    let validateEconomicCatAIBudget: LookupValidator<_> =
        fun lu os es ->
            ...
            let rec checkKey (key: string) =
                if Set.contains key budgets then
                    true
                else
            ...
            let checkEcon (econ: Node) =
                ...
                if checkKey key then
                    ...
                    Invalid(
                        ...
                        [ inv
                              ((ErrorCodes.CustomError(
                                  sprintf "Economic category %s nor any of its parents have an ai_budget" econ.Key
                              ))
                                  Severity.Warning)
                              econ ]
                    )
